		...
		if (mem.active) {
			...
			if (creep.repair(target) == ERR_NOT_IN_RANGE) {
				if (creep.room.name != mem.target_pos.roomName) {
					let next = find_next_room(
						creep.room.name,
						mem.target_pos.roomName
					)
			...
			if (
				!deposit ||
				deposit.store.getUsedCapacity(RESOURCE_ENERGY) == 0
			) {
				deposit = find_nearest_building(creep.pos, {
					filter: this.filter_deposit,
				})
				if (!deposit) {
