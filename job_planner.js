module.exports = {
	cycle: function (room) {
		...
		let assigned_maintaince_sites = _.filter(
			Object.values(Memory.jobs),
			(job) => job.type == "maintain"
		).map((job) => job.target)
		let maintenance = room
			.find(FIND_STRUCTURES, {
				filter: (structure) =>
					structure.hits < structure.hitsMax &&
					!(structure.id in assigned_maintaince_sites),
			})
	...
	init: function (room) {
		...
		for (let s in sources) {
			...
			let source_jobs = _.filter(
				room.memory.jobs,
				(job) => job.target == source_id
			)
	...
	filter_sites: function (site) {
		return (
			Object.values(Memory.jobs).filter((job) => job.target == site.id)
				.length < 1
		)
	},
}
